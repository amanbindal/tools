    ...
    async function renderTable() {
      ...
      async function lazyFetchHistory(ticker) {
        const cacheKey = `Straddle_er_${ticker}_lazy`;
        let cached = await getCache(cacheKey);
        let rsi = 0;
        let ma10 = 0;
        
        if (cached && cached.data && cached.fetchedAt && !isNewBusinessDay(cached.fetchedAt)) {
            rsi = cached.data.rsi;
            ma10 = cached.data.ma10;
        } else {
            try {
                const res = await fetchWithRetry(`https://phx.unusualwhales.com/api/companies/${ticker}/get_historical_prices?limit=50`);
                if (res && res.history && res.history.length > 9) {
                    const sortedHistory = [...res.history].sort((a, b) => new Date(a.date) - new Date(b.date));
                    const prices = sortedHistory.map(item => parseFloat(item.price || item.close_price)).filter(p => !isNaN(p));
                    if (prices.length > 14) rsi = calculateRSI(prices);
                    if (prices.length > 9) ma10 = calculateMA10(prices);
                    await setCache(cacheKey, { data: { rsi, ma10 }, fetchedAt: new Date().toISOString() });
                }
            } catch(e) {}
        }
        ...
        if (appDataItem && appDataItem.data) {
            appDataItem.data.calculatedRsi = rsi;
            appDataItem.data.calculatedMa10 = ma10;
            appDataItem.data.finalRsi = rsi;
            const price = appDataItem.data.price || 0;
            appDataItem.data.ma10Ratio = ma10 ? ((price - ma10) / ma10) * 100 : 0;
            
            const row = document.getElementById(`row-${ticker}`);
            if (row) {
                const rsiCell = row.querySelector('.rsi');
                if (rsiCell) rsiCell.innerText = rsi ? rsi.toFixed(2) : '-';
                
                const ma10Cell = row.querySelector('.ma10');
                if (ma10Cell) {
                    const ma10Text = appDataItem.data.ma10Ratio ? appDataItem.data.ma10Ratio.toFixed(2) : '-';
                    ma10Cell.innerText = ma10Text;
                    
                    const ratio = appDataItem.data.ma10Ratio;
                    ma10Cell.style.color = ratio > 0 ? '#22c55e' : (ratio < 0 ? '#ef4444' : '');
                }
            }
        }
      }
      ...
      async function processLazyQueue() {
        while (lazyQueue.length > 0) {
          ...
          await Promise.all(batch.map(t => lazyFetchHistory(t)));
    ...
    function sortTable(columnIndex, forceDesc = null) {
      ...
      appData.sort((a, b) => {
        ...
        else if (columnIndex === 18) { valA = parseFloat(a.data.price) || -999; valB = parseFloat(b.data.price) || -999; }
        else if (columnIndex === 19) { valA = a.data.finalRsi || a.data.calculatedRsi || -999; valB = b.data.finalRsi || b.data.calculatedRsi || -999; }
        else if (columnIndex === 20) { valA = parseFloat(a.data.iv_rank) || -999; valB = parseFloat(b.data.iv_rank) || -999; }
        else if (columnIndex === 21) { valA = parseFloat(a.data.beta) || -999; valB = parseFloat(b.data.beta) || -999; }
        else { valA = 0; valB = 0; }
