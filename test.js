    ...
    function renderTableBody() {
      ...
      for (const item of appData) {
        ...
        let isEntryNear = false;
        if (data.entry) {
            const absUcp = Math.abs(parseFloat(data.ucp) || 999);
            const absLcp = Math.abs(parseFloat(data.lcp) || 999);
            const absMcp = Math.abs(parseFloat(data.mcp) || 999);
            const absMa200 = Math.abs(parseFloat(data.ma200Ratio) || 999);
            
            if (data.entry === 'UCP' && absUcp <= 1.5) isEntryNear = true;
            else if (data.entry === 'LCP' && absLcp <= 1.5) isEntryNear = true;
            else if (data.entry === 'MCP' && absMcp <= 1.5) isEntryNear = true;
            else if (data.entry === 'MA200' && absMa200 <= 1.5) isEntryNear = true;
            else if (data.entry === 'LCP/MA200' && (absLcp <= 1.5 || absMa200 <= 1.5)) isEntryNear = true;
            else if (data.entry === 'Lcp-2S' && absLcp <= 1.5) isEntryNear = true;
        }
