      ...
      async function lazyFetchTargets(ticker) {
        ...
        if (cached && cached.data && cached.fetchedAt && !isNewBusinessDay(cached.fetchedAt)) {
            ...
            try {
                ...
                if (res && res.data && res.data.length > 0) {
                    ...
                    if (validRatings.length > 0) t1 = parseFloat(validRatings[0].price_target).toFixed(2);
                    if (validRatings.length > 1) t2 = parseFloat(validRatings[1].price_target).toFixed(2);
                    if (validRatings.length > 2) t3 = parseFloat(validRatings[2].price_target).toFixed(2);
        ...
        if (appDataItem && appDataItem.data) {
            ...
            if (row) {
                ...
                if (t1Cell) t1Cell.innerText = t1;
                
                const t2Cell = row.querySelector('.t2');
                if (t2Cell) t2Cell.innerText = t2;
                
                const t3Cell = row.querySelector('.t3');
                if (t3Cell) t3Cell.innerText = t3;
            }
        }
      }
    ...
    function sortTable(columnIndex, forceDesc = null) {
      ...
      appData.sort((a, b) => {
        ...
        else if (columnIndex === 22) { valA = parseFloat(a.data.t1) || -999; valB = parseFloat(b.data.t1) || -999; }
        else if (columnIndex === 23) { valA = parseFloat(a.data.t2) || -999; valB = parseFloat(b.data.t2) || -999; }
        else if (columnIndex === 24) { valA = parseFloat(a.data.t3) || -999; valB = parseFloat(b.data.t3) || -999; }
        else { valA = 0; valB = 0; }
